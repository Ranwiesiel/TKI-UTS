...
for data in content :
	tf={} 
	#clean and list word
	clean_title = clean_str(data['univ_name'])
...
for word in df_data:
	...
	for data in content:
		...
		if word in tf_data[data['univ_name']] :
			tf_value = tf_data[data['univ_name']][word]

		weight = tf_value * idf_data[word]

		doc = {
			'title' : data['univ_name'],
			'dosen' : data['jumlah_dosen'],
			'score' : weight
		}
